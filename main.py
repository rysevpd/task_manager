...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.search_menu.add_command(label="Поиск по ответсвенному", command=self.search_user)

        self.file_menu.add_command(label="Добавить категорию", command=self.add_category)
        self.file_menu.add_command(label="Добавить пользователя", command=self.add_user)
        self.file_menu.add_command(label="Добавить задачу", command=self.add_task)
        self.file_menu.add_command(label="Показать пользователей", command=self.list_users)
...
class AddUser(tk.Toplevel):
    ...
    def init_user(self):
        ...
        self.entry_user = Entry(frame_1, justify="right")
        self.entry_user.pack(fill=X, padx=5, expand=True)
        self.entry_login = Entry(frame_2, justify="right")
        self.entry_login.pack(fill=X, padx=5, expand=True)
        self.entry_pas = Entry(frame_3, justify="right")
        self.entry_pas.pack(fill=X, padx=5, expand=True)
        self.entry_post = Entry(frame_5, justify="right")
        self.entry_post.pack(fill=X, padx=5, expand=True)
        self.entry_number = Entry(frame_6, justify="right")
        self.entry_number.pack(fill=X, padx=5, expand=True)
        self.entry_address = Entry(frame_7, justify="right")
        self.entry_address.pack(fill=X, padx=5, expand=True)

        self.entry_role = ttk.Combobox(frame_4, values=[u'admin', u'user'])
        self.entry_role.current(0)
        self.entry_role.pack(fill=X, padx=5, expand=True)

        frame_8 = Frame(self, bg="#48466d")
        frame_8.pack(fill=X)
...
class Category(tk.Toplevel):
    ...
    def init_category(self):
        ...
        self.entry_category = Entry(frame_1, justify="right")
